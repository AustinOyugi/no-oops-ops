...
func TestParseReleaseArgsAcceptsAllForMultiServiceManifest(t *testing.T) {
	manifestPath := filepath.Join(t.TempDir(), "app.yml")
	if err := os.WriteFile(manifestPath, []byte("services:\n  api:\n    image: api\n  worker:\n    image: worker\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	environment, path, services, deployAfterRelease, err := parseReleaseArgs([]string{"--deploy", "prod", "app", "--all"}, resolve)
	if err != nil {
		t.Fatalf("parseReleaseArgs returned error: %v", err)
	}
	if environment != "prod" || path != manifestPath || strings.Join(services, ",") != "api,worker" || !deployAfterRelease {
		t.Errorf("parseReleaseArgs = (%q, %q, %v, %t), want all services with deploy", environment, path, services, deployAfterRelease)
	}
}
