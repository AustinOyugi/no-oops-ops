...
func TestFilesystemDeploymentStorePreviousReturnsDeploymentBeforeLatest(t *testing.T) {
	...
	for _, deployment := range []Deployment{
		{App: "sample", Environment: "prod", CreatedAt: first, ReleaseTag: "first", ReleaseImage: "registry/sample:first"},
		{App: "sample", Environment: "prod", CreatedAt: second, ReleaseTag: "second", ReleaseImage: "registry/sample:second"},
	} {
	...
	previous, err := store.Previous(cfg, "sample", "prod")
	if err != nil {
...
func TestFilesystemDeploymentStorePreviousRequiresTwoDeployments(t *testing.T) {
	...
	if _, err := store.Save(cfg, Deployment{App: "sample", Environment: "prod", CreatedAt: time.Now().UTC(), ReleaseTag: "only"}); err != nil {
		t.Fatalf("save deployment: %v", err)
	}

	_, err := store.Previous(cfg, "sample", "prod")
