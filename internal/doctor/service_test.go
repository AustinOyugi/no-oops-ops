...
func TestRunInactiveSwarmSkipsSwarmDependentChecks(t *testing.T) {
	...
	for _, name := range []string{"shared_network", "registry_service", "registry_config", "registry_stack"} {
...
func TestRunDockerUnavailableSkipsDockerDependentChecks(t *testing.T) {
	...
	if result.Count(StatusSkip) != 5 {
		t.Errorf("skipped checks = %d, want 5", result.Count(StatusSkip))
	}
}
