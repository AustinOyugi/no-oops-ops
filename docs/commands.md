...
`uninstall` does not remove the installed CLI executable. `make uninstall` additionally removes the repository-local `.bin/noops` after teardown succeeds. `uninstall --purge` removes only the workspace `.noops/state` and `.noops/data` directories.
